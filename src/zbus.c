...
/* --- Adicione estes para o Zbus --- */
#include <zephyr/zbus/zbus.h>
/* --- Adicione estes para o Shell --- */
#include <zephyr/shell/shell.h>
/* --- Adicione estes para o Sensor MPU6050 --- */
...
//     const struct fft_msg *fft = zbus_chan_const_msg(chan);
//     cnt_listener++;
//     if (cnt_listener > 10) { // <--- A cada 11 publicações, ele imprime
//         cnt_listener = 0;
//         LOG_INF("From listener -> FFT: ...");
//     }
// }
...
static void send_result_task(void)
{
    ...
    while (!zbus_sub_wait(&task_sub, &chan, K_FOREVER)) {
        struct fft_msg fft;
        zbus_chan_read(&fft_data_chan, &fft, K_MSEC(100));

        if (fft.harmonic_1st > LIMITE_CRITICO) {
            // Em vez de chamar send_to_cloud() agora, vamos apenas logar
            // para testar se a lógica de detecção está funcionando.
            LOG_INF("Alerta! FFT Harmonica 1 ultrapassou o limite: %.2f", (double)fft.harmonic_1st);
        }
    }
}
...
static void fft_task(void)
{
    ...
    if (!device_is_ready(dev)) {
        LOG_ERR("Sensor MPU6050 não está pronto!");
        return;
    }
    ...
    while (1) {
        ...
        for(int i = 0; i < 256; i++) {
            ...
            float val = sensor_value_to_double(&accel_x);
            filter_in[i] = val;
            sum += val; // Soma para calcular a média (offset)
            
            k_usleep(1000); 
        }

        // 2. Remoção do Offset (Componente DC)
        float mean = sum / 256.0f;
        for(int i = 0; i < 256; i++) {
            ReIm[i*2] = filter_in[i] - mean; // Subtrai a gravidade/offset
            ReIm[i*2 + 1] = 0.0f;            // Parte imaginária
        }
        ...
        // Aplicação do filtro (mantendo sua lógica original)
        arm_biquad_cascade_df2T_f32(&iir_inst, filter_in, filter_out, 256);
        
        // ... (se necessário, re-processe a FFT após o filtro se for o seu objetivo)

        // 4. Publicação via ZBUS
        fft.harmonic_cc = mod[0] / 2; // Componente DC residual
...
// Lembre-se de ajustar a prioridade para um valor menor (ex: 2 ou 3) para ser Hard RT 
K_THREAD_DEFINE(fft_calculation_task_id, CONFIG_MAIN_STACK_SIZE, fft_task, NULL, NULL, NULL, 3, 0, 0);
...
static int cmd_show_fft(const struct shell *shell, size_t argc, char **argv)
{
    ...
    // Tenta ler o último valor publicado no canal
    if (zbus_chan_read(&fft_data_chan, &fft, K_MSEC(100)) == 0) {
        shell_print(shell, "--- Últimos Dados de FFT ---");
...
// Registra o comando "show_fft" no shell
